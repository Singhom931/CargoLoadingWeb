...
class Bin:
    ...
    def __init__(self, partno,name, WHD, max_weight,corner=0,put_type=1):
        ''' '''
        self.partno = partno
        self.name = name
        self.width = WHD[0]
...
class Packer:
    ...
    def gravityCenter(self,bin):
        ...
        result = []
        for i in r :
            if sum(r) == 0: result.append(0)
            else: result.append(round(i / sum(r) * 100,2))
        return result
...
class PainterPlotly:
    ...
    def to_float(self, value):
        ...
        if isinstance(value, list):
            return list(self.to_float(v) for v in value)
        elif isinstance(value, tuple):
            return tuple(self.to_float(v) for v in value)
        return value

    def _plotCube(self, fig, x, y, z, dx, dy, dz, color='red', text="", alpha=0.5, showlegend=True):
        """Helper function to plot a cube using Plotly."""
        ...
        vertices = list(zip(*vertices))  # Transpose for Plotly
    ...
    def _plotCylinder(self, fig, x, y, z, dx, dy, dz, color='red', text="", alpha=0.5):
        """Helper function to plot a cylinder using Plotly."""
        theta = np.linspace(0, 2 * np.pi, 50)
        z_cylinder = np.linspace(z, z + dz, 50)
    ...
    def plotBoxAndItems(self, title="", alpha=0.2, write_num=False, canvas_height=450, canvas_width=700,showlegend=True):
        """Main method to plot the bin and items."""
        fig = go.Figure()

        # Plot the bin (outer box)
        self._plotCube(fig, 0, 0, 0, self.to_float(self.width), self.to_float(self.height), self.to_float(self.depth), color='black', alpha=0.1,showlegend=showlegend)

        plots = []
        # Plot each item inside the bin
        for item in self.items:
        # def plot_item(item):
            # """Helper function to plot an item and generate its base64 image."""
            x, y, z = self.to_float(item.position)
            w, h, d = self.to_float(item.getDimension())
            color = item.color
            text = item.partno if write_num else ""

            if item.typeof == 'cube':
                self._plotCube(fig, x, y, z, w, h, d, color=color, text=text, alpha=alpha)
            elif item.typeof == 'cylinder':
                self._plotCylinder(fig, x, y, z, w, h, d, color=color, text=text, alpha=alpha)

            # buf = BytesIO()
            # fig.write_image(buf, format='jpeg', scale=2,)
            # buf.seek(0)
        ...
        # # Parallel processing for items
        # with ThreadPoolExecutor() as executor:
        #     plots = list(executor.map(plot_item, self.items))

        # Set equal axes and titles
        fig.update_layout(
            scene=dict(
                xaxis=dict(nticks=4, range=[0, self.to_float(self.width)]),
                yaxis=dict(nticks=4, range=[0, self.to_float(self.height)]),
                zaxis=dict(nticks=4, range=[0, self.to_float(self.depth)]),
                # aspectmode='cube'
                aspectmode = 'data'
            ),
            title=title,
            width=canvas_width,  # Increase the width of the canvas
            height=canvas_height
        )

        # Render Plotly figure as base64
        # img_str = base64.b64encode(fig.to_image(format="png")).decode("utf-8")
        return [fig, plots]
